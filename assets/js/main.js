...
if (window.innerWidth > 1023)
    Array.from($('.heading__menu-item')).forEach(e => e.classList.add('active'));

window.onresize = () => {
    const headingMenuItems = Array.from($('.heading__menu-item'));
    if (window.innerWidth > 1023) {
        headingMenu.style.height = '84px';
        headingMenuItems.forEach(e => e.classList.add('active'));
    } else {
        headingMenu.style.height = '0';
        headingMenuItems.forEach(e => e.classList.remove('active'));
    }
};
